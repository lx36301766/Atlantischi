...
public class FileDownloader implements Runnable {
    ...
    public boolean start(String url, File local_file, boolean resume) {
        ...
        try {
            ...
            Xlog.e(TAG, "file not found exception");
            return false;
        } catch (IOException e) {
            _error_status = ERR_IO_ERROR;
            Xlog.e(TAG, "io exception");
            return false;
        }
...
//            _handler = null;
    ...
    public void run() {
        try {
            Xlog.d(TAG, "start downloading... " + _url);
            URI uri = new URI(_url);
            if (uri.getHost() == null || uri.getPath() == null) {
                Xlog.e(TAG, "URI no host/path error!! " + _url);
                this._error_status = ERR_INTERNAL_ERROR;
                this.sendMessage(MSG_ERROR);
                Xlog.d(TAG, "MSG_ERROR---111");
                return;
            }
            HttpGet get_url = new HttpGet(uri);
            if (_resume_pos > 0) {
                Xlog.d(TAG, "resuming... bytes=" + _resume_pos + "-");
            ...
            Xlog.d(TAG, "response " + code + " " + _status.getReasonPhrase());
            if (code != 200 && code != 206) {
                Xlog.e(TAG, "HTTP error!! ");
                this._error_status = ERR_HTTP_RESPONSE_ERROR;
                this.sendMessage(MSG_ERROR);
                Xlog.d(TAG, "MSG_ERROR---222");
                return;
            }
            ...
            if (_resume_pos > 0 && code == 206) {
                Xlog.d(TAG, "check content-range...");
                // resume downloading
                Header[] header = resp.getHeaders("Content-Range");
                if (header == null || header.length != 1) {
                    Xlog.e(TAG, "resume: HTTP error, no content-range header!");
                    this._error_status = ERR_INTERNAL_ERROR;
                    this.sendMessage(MSG_ERROR);
                    Xlog.d(TAG, "MSG_ERROR---333");
                    return;
                }
                {
                    ...
                    if (!m.matches()) {
                        Xlog.e(TAG, "resume: unsupported content-range:" + content_range_val);
                        this._error_status = ERR_INTERNAL_ERROR;
                        this.sendMessage(MSG_ERROR);
                        Xlog.d(TAG, "MSG_ERROR---444");
                        return;
                    }
                    ...
                    if (begin_pos != _resume_pos) {
                        Xlog.e(TAG, "resume: file range invalid");
                        this._error_status = ERR_RESUME_CHECK_FAILED;
                        this.sendMessage(MSG_ERROR);
                        Xlog.d(TAG, "MSG_ERROR---555");
                        return;
                    }
                    ...
                    while (size < _resume_sign.length) {
                        if (Thread.interrupted()) {
                            Xlog.w(TAG, "download interrupted!!");
                            return;
                        }
                    ...
                    if (size != _resume_sign.length || !Arrays.equals(sign_buffer, _resume_sign)) {
                        Xlog.e(TAG, "resume data check failed!!");
                        this._error_status = ERR_RESUME_CHECK_FAILED;
                        this.sendMessage(MSG_ERROR);
                        Xlog.d(TAG, "MSG_ERROR---666");
                        return;
                    }
                    Xlog.d(TAG, "continue download from " + _resume_pos);
                }
            } else {
                Xlog.d(TAG, "download from begining...");
                _file.seek(0);
            }
            ...
            Xlog.e(TAG, "io exception", e);
            this._error_status = ERR_IO_ERROR;
            this.sendMessage(MSG_ERROR);
            Xlog.d(TAG, "MSG_ERROR---777", e);
        } catch (NumberFormatException e) {
            Xlog.e(TAG, "number format exception");
            this._error_status = ERR_INTERNAL_ERROR;
            this.sendMessage(MSG_ERROR);
            Xlog.d(TAG, "MSG_ERROR---888", e);
        } catch (URISyntaxException e) {
            Xlog.e(TAG, "URI syntax error!! " + _url);
            this._error_status = ERR_INTERNAL_ERROR;
            this.sendMessage(MSG_ERROR);
            Xlog.d(TAG, "MSG_ERROR---999", e);
        } catch (Exception e) {
            Xlog.e(TAG, "unknown exception!! " + e.getMessage());
            this._error_status = ERR_INTERNAL_ERROR;
            this.sendMessage(MSG_ERROR);
            Xlog.d(TAG, "MSG_ERROR---000", e);
        } finally {
            _client = null;
        }
    }

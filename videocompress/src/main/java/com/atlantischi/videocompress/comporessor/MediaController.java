package com.atlantischi.videocompress.comporessor;
...
public class MediaController {
    ...
    private static final String TAG = "LxMediaController";
     ...
     */
    public static final String VIDEO_COMPRESSOR_APPLICATION_DIR_NAME = "lx";

    /**
     * Application folder for video files
     */
    public static final String VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR = "/Videos/";
    ...
    public native static int convertVideoFrame(ByteBuffer src, ByteBuffer dest, int destFormat, int width, int height,
                                               int padding, int swap);
    ...
    @TargetApi(16)
    private long readAndWriteTrack(MediaExtractor extractor, MP4Builder mediaMuxer, MediaCodec.BufferInfo info,
                                   long start, long end, File file, boolean isAudio) throws Exception {
    ...
    public static final class CompressResult {
        ...
        public CompressResult(boolean r, String p) {
            mResult = r;
            mPath = p;
        }
    ...
    public static File cacheFile = new File(
            Environment.getExternalStorageDirectory()
                    + File.separator
                    + VIDEO_COMPRESSOR_APPLICATION_DIR_NAME
                    + VIDEO_COMPRESSOR_COMPRESSED_VIDEOS_DIR, "COMPRESS_VIDEO_CACHE.mp4");

    @TargetApi(16)
    public CompressResult convertVideo(final String path) {
        CompressResult res = new CompressResult(false, cacheFile.getAbsolutePath());
        ...
        int originalWidth = 0;//Integer.valueOf(width);
        int originalHeight =0;// Integer.valueOf(height);

        int rotationValue = 0;//Integer.valueOf(rotation);
        try {
        ...
        resultHeight = Math.min(originalHeight, originalWidth) * 640 / Math.max(originalHeight, originalWidth);
        int bitrate = BIT_RATE;//450000;
        int rotateRender = 0;
                    ...
                    if (videoIndex >= 0) {
                        ...
                        try {
                            ...
                            if (Build.VERSION.SDK_INT < 18) {
                                ...
                                Log.e(TAG, "codec = " + codecInfo.getName() + " manufacturer = " + manufacturer
                                        + "device = " + Build.MODEL);
                            } else {
                            ...
                            outputFormat.setInteger(MediaFormat.KEY_FRAME_RATE, VIDEO_FRAMERATE);
                            outputFormat.setInteger(MediaFormat.KEY_I_FRAME_INTERVAL, I_INTERVAL);
                            if (Build.VERSION.SDK_INT < 18) {
                                    ...
                                    if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                                        ...
                                        if (info.size > 1) {
                                            if ((info.flags & MediaCodec.BUFFER_FLAG_CODEC_CONFIG) == 0) {
                                                if (mediaMuxer
                                                        .writeSampleData(videoTrackIndex, encodedData, info, false)) {
                                                ...
                                                for (int a = info.size - 1; a >= 0; a--) {
                                                    if (a > 3) {
                                                        if (csd[a] == 1 && csd[a - 1] == 0 && csd[a - 2] == 0
                                                                && csd[a - 3] == 0) {
                                                ...
                                                MediaFormat newFormat = MediaFormat
                                                        .createVideoFormat(MIME_TYPE, resultWidth, resultHeight);
                                        ...
                                        if (decoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
                                            ...
                                            if (startTime > 0 && videoTime == -1) {
                                                if (info.presentationTimeUs < startTime) {
                                                    doRender = false;
                                                    Log.e(TAG,
                                                            "drop frame startTime = " + startTime + " present time = "
                                                ...
                                                if (!errorWait) {
                                                    if (Build.VERSION.SDK_INT >= 18) {
                                                        ...
                                                        if (inputBufIndex >= 0) {
                                                            ...
                                                            encoder.queueInputBuffer(inputBufIndex, 0, bufferSize,
                                            ...
                                            if ((info.flags & MediaCodec.BUFFER_FLAG_END_OF_STREAM) != 0) {
                                                ...
                                                if (Build.VERSION.SDK_INT >= 18) {
                                                    ...
                                                    int inputBufIndex = encoder.dequeueInputBuffer(TIMEOUT_USEC);
                                                    if (inputBufIndex >= 0) {
                                                        encoder.queueInputBuffer(inputBufIndex, 0, 1,
                                                                info.presentationTimeUs,
                                                                MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                                                    }
                                                }

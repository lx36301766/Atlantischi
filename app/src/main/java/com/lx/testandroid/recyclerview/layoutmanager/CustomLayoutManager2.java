...
public class CustomLayoutManager2 extends RecyclerView.LayoutManager {
     ...
     * @param orientation {@link State#orientation}
     * @param itemLineCount {@link State#itemLineCount}
     */
    public CustomLayoutManager2(@Orientation int orientation, int itemLineCount) {
    ...
    public void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        for (int i = 0; i < getItemCount(); i++) {
            ...
            if (occupiedLineBlock > mState.itemLineCount) {
                if (mState.orientation == HORIZONTAL) {
                    offsetX += currentLineSpreadLength; // 横向偏移当前line的最大宽度
                    offsetY = 0; // 纵向重置到0
                    mState.contentWidth += currentLineSpreadLength; // contentWidth增加当前line的最大宽度
                }
                else {
                    offsetX = 0; // 横向重置到0
        ...
        if (mState.orientation == HORIZONTAL) {
            mState.contentWidth += currentLineSpreadLength; // contentWidth增加最后line的最大宽度
            mState.contentHeight = maxLineLength; // contentHeight设为所有line的最大高度
        }
        else {
    ...
    public boolean canScrollHorizontally() {
        return mState.canScrollHorizontal;
    }
    ...
    public int scrollHorizontallyBy(int dx, RecyclerView.Recycler recycler, RecyclerView.State state) {
         ...
         * 限制滑动距离的最小值和最大值
         */
        if (mState.scrolledX + dx < 0) {
            willScroll = -mState.scrolledX;
        }
        else if (mState.scrolledX + dx > mState.contentWidth - getHorizontalSpace()) {
            willScroll = mState.contentWidth - getHorizontalSpace() - mState.scrolledX;
        }
    ...
    public boolean canScrollVertically() {
        return mState.canScrollVertical;
    }
    ...
    public void scrollToPosition(int position) {
        ...
        mState.scrolledX = mState.itemsFrames.get(position).left;
    ...
    public void smoothScrollToPosition(RecyclerView recyclerView, RecyclerView.State state, int position) {
        ...
        LinearSmoothScroller scroller = new LinearSmoothScroller(recyclerView.getContext()) {
            ...
            public PointF computeScrollVectorForPosition(int targetPosition) {
                int oldScrollX = mState.scrolledX;
                int oldScrollY = mState.scrolledY;

                mState.scrolledX = mState.itemsFrames.get(targetPosition).left;
                mState.scrolledY = mState.itemsFrames.get(targetPosition).top;
                fixScrollOffset();

                int newScrollX = mState.scrolledX;
                int newScrollY = mState.scrolledY;

                mState.scrolledX = oldScrollX;
    ...
    private void layoutItems(RecyclerView.Recycler recycler, RecyclerView.State state) {
        ...
        Rect displayFrame = new Rect(mState.scrolledX, mState.scrolledY, mState.scrolledX + getHorizontalSpace(), mState.scrolledY + getVerticalSpace());
            ...
            if (Rect.intersects(displayFrame, mState.itemsFrames.get(i))) {
                ...
                if (!mState.itemsAttached.get(i)) {
                    ...
                    layoutDecorated(scrap,
                            frame.left - mState.scrolledX,
                            frame.top - mState.scrolledY,
                            frame.right - mState.scrolledX,
...
//    }
     ...
     */
    public CustomLayoutManager2 setCanScrollHorizontal(boolean canScrollHorizontal) {
        mState.canScrollHorizontal = canScrollHorizontal;
        return this;
    }
     ...
     */
    public CustomLayoutManager2 setCanScrollVertical(boolean canScrollVertical) {
        mState.canScrollVertical = canScrollVertical;
        return this;
    }
    ...
    public CustomLayoutManager2 setOrientation(@Orientation int orientation) {
        if (orientation != HORIZONTAL && orientation != VERTICAL) {
            throw new IllegalArgumentException("invalid orientation:" + orientation);
        }
        assertNotInLayoutOrScroll(null);
        if (orientation == mState.orientation) {
            return this;
        }
    ...
    public int getOrientation() {
        return mState.orientation;
    }

    public CustomLayoutManager2 setItemLineCount(int itemLineCount) {
        assertNotInLayoutOrScroll(null);
    ...
    private void fixScrollOffset() {
        if (mState.contentWidth == getHorizontalSpace()) {
            mState.scrolledX = 0;
        }
        if (mState.scrolledX > (mState.contentWidth - getHorizontalSpace())) {
            mState.scrolledX = mState.contentWidth - getHorizontalSpace();
        }

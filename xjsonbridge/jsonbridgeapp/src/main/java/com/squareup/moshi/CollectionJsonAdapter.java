...
abstract class CollectionJsonAdapter<C extends Collection<T>, T> extends JsonAdapter<C> {
  public static final JsonAdapter.Factory FACTORY = new JsonAdapter.Factory() {
    ...
    JsonAdapter<?> create(
        Type type, Set<? extends Annotation> annotations, com.squareup.moshi.Moshi moshi) {
  ...
  static <T> JsonAdapter<Collection<T>> newArrayListAdapter(Type type, com.squareup.moshi.Moshi moshi) {
    Type elementType = Types.collectionElementType(type, Collection.class);
    JsonAdapter<T> elementAdapter = moshi.adapter(elementType);
    return new com.squareup.moshi.CollectionJsonAdapter<Collection<T>, T>(elementAdapter) {
  ...
  static <T> JsonAdapter<Set<T>> newLinkedHashSetAdapter(Type type, Moshi moshi) {
    Type elementType = Types.collectionElementType(type, Collection.class);
    JsonAdapter<T> elementAdapter = moshi.adapter(elementType);
    return new com.squareup.moshi.CollectionJsonAdapter<Set<T>, T>(elementAdapter) {
